...
from file_editor.core import FileEditor
...
<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">
    ...
    <rect x="10" y="10" width="50" height="50" fill="red" id="test-rect"/>
    <text x="50" y="150" id="test-text" font-size="16">Hello World</text>
</svg>'''
...
class TestFileEditor:
    ...
    def test_load_svg_file(self, sample_svg):
    ...
    def test_load_xml_file(self, sample_xml):
        """Test loading XML file"""
        editor = FileEditor(sample_xml)
        assert editor.file_type == 'xml'
        assert editor.tree is not None

    def test_load_html_file(self, sample_html):
    ...
    @pytest.mark.skipif(not hasattr(FileEditor, 'find_by_xpath'),
                        reason="XPath support requires lxml")
    def test_xpath_query_text(self, sample_svg):
    ...
    @pytest.mark.skipif(not hasattr(FileEditor, 'find_by_xpath'),
                        reason="XPath support requires lxml")
    def test_xpath_query_attribute(self, sample_svg):
    ...
    @pytest.mark.skipif(not hasattr(FileEditor, 'find_by_xpath'),
                        reason="XPath support requires lxml")
    def test_set_element_text(self, sample_svg):
        """Test setting element text"""
        editor = FileEditor(sample_svg)
        success = editor.set_element_text("//text[@id='test-text']", "Updated Text")
        assert success is True

        # Verify the change
        updated_text = editor.get_element_text("//text[@id='test-text']")
        assert updated_text == "Updated Text"

    @pytest.mark.skipif(not hasattr(FileEditor, 'find_by_xpath'),
                        reason="XPath support requires lxml")
    def test_set_element_attribute(self, sample_svg):
        """Test setting element attribute"""
        editor = FileEditor(sample_svg)
        success = editor.set_element_attribute("//rect[@id='test-rect']", "fill", "blue")
        assert success is True

        # Verify the change
        updated_fill = editor.get_element_attribute("//rect[@id='test-rect']", "fill")
        assert updated_fill == "blue"

    @pytest.mark.skipif(not hasattr(FileEditor, 'find_by_xpath'),
                        reason="XPath support requires lxml")
    def test_list_elements(self, sample_xml):
    ...
    def test_backup_creation(self, sample_svg):
    ...
    def test_save_file(self, sample_svg):
        """Test saving file"""
        editor = FileEditor(sample_svg)
    ...
    def test_invalid_xpath(self, sample_svg):
    ...
    def test_nonexistent_element(self, sample_svg):
